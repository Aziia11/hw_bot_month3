...
def sql_create():
    ...
    connection.execute(
        ...
        (photo TEXT, telegram_account_id TEXT,username PRIMARY KEY, first_name TEXT,last_name TEXT)
        """
    )
    connection.commit()
...
async def user_select(message):
    for show in cursor.execute("""SELECT * FROM users""").fetchall():
        await bot.send_photo(message.chat.id,
                             show[0],
                             caption=f'telegram_account_id {show[1]}\n'
                                     f'username: {show[2]}\n'
                                     f'first_name {show[3]}\n'
                                     f'last_name: {show[4]}')
